import { InputType, Field, Float } from '@nestjs/graphql';
import {
  IsNotEmpty,
  IsString,
  Length,
  IsOptional,
  IsNumber,
} from 'class-validator';

@InputType()
export class CreateUserInput {
  @Field()
  @IsNotEmpty({ message: 'El DNI es obligatorio.' }) // No vacío
  @IsString()
  @Length(8, 8, { message: 'El DNI debe tener 8 caracteres.' }) // Longitud exacta de 8 caracteres
  dni: string;

  @Field()
  @IsNotEmpty({ message: 'El nombre es obligatorio.' })
  @IsString()
  @Length(1, 100, { message: 'El nombre no puede exceder los 100 caracteres.' }) // Longitud máxima de 100 caracteres
  name: string;

  @Field()
  @IsNotEmpty({ message: 'El apellido es obligatorio.' })
  @IsString()
  @Length(1, 100, {
    message: 'El apellido no puede exceder los 100 caracteres.',
  })
  last_name: string;

  @Field({ nullable: true })
  @IsOptional()
  @Length(9, 9, { message: 'El teléfono debe tener 9 caracteres.' })
  phone?: string;

  @Field(() => Float)
  @IsNotEmpty({ message: 'El salario es obligatorio.' })
  @IsNumber(
    { maxDecimalPlaces: 2 },
    {
      message:
        'El salario debe ser un número decimal con máximo dos decimales.',
    },
  ) // Verificación para salario
  salary: number;

  @Field()
  @IsNotEmpty({ message: 'La contraseña es obligatoria.' })
  @IsString()
  @Length(8, 50, { message: 'La contraseña debe tener al menos 8 caracteres.' }) // Requiere al menos 8 caracteres
  password: string; // Contraseña no en claro, será hasheada después
}
